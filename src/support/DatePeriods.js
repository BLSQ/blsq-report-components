...
class DatePeriods {
  ...
  static sixMonthlyName(dhis2period) {
    ...
    const monthsPeriod = this.split(dhis2period, SIX_MONTHLY)
    return (
      [monthsPeriod[0], monthsPeriod[5]]
        .map(monthPeriod => MONTH_NAMES[monthPeriod - 1])
        .join(' - ') +
      ' ' +
      yearPeriod
    )
  ...
  static sixMonthlyMatchQuarter(year, quarterStr) {
    let quarter = parseInt(quarterStr, 0)
    let sixMonthly = []
    if (quarter === 1 || quarter === 2) {
      sixMonthly = [1]
    ...
    return sixMonthly.map(s => year + 'S' + s)
  }
  ...
  static split(period, splitType) {
    ...
    if (period.includes('S')) {
      return this.splitSixMonthInMonths(period, splitType)
    }
    if (period.length === 6) {
  ...
  static splitYearSixMonth(period, splitType) {
    ...
    if (splitType === SIX_MONTHLY) {
      return this.sixMonthlyMatchQuarter(year, quarter)
    }
  ...
  static splitSixMonthInMonths(period, splitType) {
    let year = parseInt(period.slice(0, 4), 0)
    let sixMonth = parseInt(period.slice(5, 6), 0)
    if (splitType === SIX_MONTHLY) {
      return SIX_MONTHLY_TO_MONTHS[sixMonth]
    }
    if (splitType === YEARLY) {
      return ['' + year]
    }
